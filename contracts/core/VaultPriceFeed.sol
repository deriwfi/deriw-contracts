...
contract VaultPriceFeed is IVaultPriceFeed {
    ...
    function setSecondaryPriceFeed(address _secondaryPriceFeed) external onlyGov {
        require(_secondaryPriceFeed != address(0), "_secondaryPriceFeed err");
    ...
    function setTokenConfig(
        ...
        require(_priceFeed != address(0), "_priceFeed err");
    ...
    function getPrimaryPrice(address _token, bool _maximise) public override view returns (uint256) {
        ...
        uint256 price = 0;
        uint80 roundId = priceFeed.latestRound(_token);
        ...
        for (uint80 i = 0; i < priceSampleSpace; i++) {
            ...
            if (i == 0) {
                int256 _p = priceFeed.latestAnswer(_token);
                require(_p > 0, "VaultPriceFeed: invalid price");
                p = uint256(_p);
            } else {
                (, int256 _p, , ,) = priceFeed.getRoundData(_token, roundId - i);
                require(_p > 0, "VaultPriceFeed: invalid price");
                p = uint256(_p);
            }
    ...
    function getLatestPrimaryPrice(address _token) public override view returns (uint256) {
        ...
        int256 price = priceFeed.latestAnswer(_token);
