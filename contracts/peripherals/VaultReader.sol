...
contract VaultReader {
    function getVaultTokenInfoV3(address _vault, address _positionRouter, address _weth, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 5] = positionRouter.maxGlobalShortSizes(token);
            amounts[i * propsLength + 6] = vaultFor.getMinPrice(token);
            amounts[i * propsLength + 7] = vaultFor.getMaxPrice(token);
            amounts[i * propsLength + 8] = vaultFor.guaranteedUsd(token, usdt);
